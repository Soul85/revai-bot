...
class Help(commands.Cog):
    ...
    async def help(self, ctx, search:str = None):
        ...
        if search in [cog.qualified_name for cog in cogs]:
            embed = discord.Embed(title=search.capitalize(), color=0xf26925)
            ...
            embed = discord.Embed(title=command.qualified_name.capitalize(), description=command.help, color=0xf26925)
            embed.add_field(name='**Usage**', value=command.usage)
        else:
            embed = discord.Embed(title='Help', color=0xf26925)
            for cog in cogs:
